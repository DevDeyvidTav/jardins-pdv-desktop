...
[void][PdvOk]::EscapeCommFunction($h, 5)
[void][PdvOk]::EscapeCommFunction($h, 3)
Start-Sleep -Milliseconds 400
$bytes = [byte[]](0x1b, 0x40) + [Text.Encoding]::ASCII.GetBytes("JARDINS - PORTA OK`n`n`n") + [byte[]](0x1b, 0x69)
$w = 0
$ok = [PdvOk]::WriteFile($h, $bytes, $bytes.Length, [ref]$w, [IntPtr]::Zero)
Write-Output ('WRITE ok=' + $ok + ' written=' + $w + ' win32=' + [Runtime.InteropServices.Marshal]::GetLastWin32Error())
Start-Sleep -Milliseconds 300
[void][PdvOk]::CloseHandle($h)
